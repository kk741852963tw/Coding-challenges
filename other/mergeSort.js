 ...
 */
 ...
 var mergeSort = function(array) {
  ...
  var finalresult = array.slice();
  var n = finalresult.length;
  for (var i = 0; i < finalresult.length; i++) {
    finalresult[i] = [finalresult[i]];
  }
  var tempmethod = function(input) {
    var result = [];
    var left = 0;
    var right = 0;
    while (left < input[0].length && right < input[1].length) {
        if (input[0][left] < input[1][right]) {
          result.push(input[0][left]);
           left++;
        } else {
          result.push(input[1][right]);
          right++;
        }
      }
    return result.concat(input[0].slice(left)).concat(input[1].slice(right));
  };
  while (n > 0) {
    var temp = [];
    if (finalresult.length % 2 === 0) {
      for (var i = 0; i < finalresult.length / 2; i++) {
        temp.push(tempmethod([finalresult[2 * i], finalresult[2 * i + 1]]));
      }
    } else {
      for (var i = 0; i < finalresult.length / 2 - 1; i++) {
        temp.push(tempmethod([finalresult[2 * i], finalresult[2 * i + 1]]));
      }
      temp.push(finalresult[finalresult.length - 1]);
    }
    n = Math.floor(n / 2);
    finalresult = temp.slice();
  }
